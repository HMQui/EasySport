import React, { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
    Select,
    SelectContent,
    SelectGroup,
    SelectItem,
    SelectLabel,
    SelectTrigger,
    SelectValue
} from "@/components/ui/select";
import {
    Dialog,
    DialogClose,
    DialogContent,
    DialogDescription,
    DialogFooter,
    DialogHeader,
    DialogTitle
} from "@/components/ui/dialog";
import { InputOTP, InputOTPGroup, InputOTPSlot } from "@/components/ui/input-otp";
import SignUpBgLg from "@/assets/images/bg/SignUpBgLg.jpg";
import { validateInput } from "@/utils/validateInput";
import { LoaderCircle } from "lucide-react";
import axiosPublic from "@/lib/axiosPublic";
import { Link, useNavigate } from "react-router-dom";
import { fetchVerifyEmail } from "@/api/auth.api";

function SignUp() {
    const naviagte = useNavigate();
    const [valueSignUp, setValueSignUp] = useState({
        email: "",
        password: "",
        confirmPassword: "",
        name: "",
        phone: "",
        role: "user"
    });
    const [verifyCode, setVerifyCode] = useState("");
    const [helpText, setHelpText] = useState({
        signUp: "",
        resendCode: ""
    });
    const [openVerifyDialog, setOpenVerifyDialog] = useState(false);
    const [loading, setLoaing] = useState({
        signUp: false,
        resendCode: false
    });

    useEffect(() => {
        document.title = "Sign Up - EasySport";
    }, []);

    const handleChangeInput = (value: string, fieldName: string) => {
        setValueSignUp((prev) => ({ ...prev, [fieldName]: value }));
        setHelpText((prev) => ({ ...prev, signUp: "" }));
    };

    const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();

        const email = valueSignUp.email;
        const password = valueSignUp.password;
        const confirmPassword = valueSignUp.confirmPassword;
        const name = valueSignUp.name;
        const phone = valueSignUp.phone;

        const validator = new validateInput();

        let isValid: string | boolean = validator.email(email);
        if (isValid != true) {
            setHelpText((prev) => ({ ...prev, signUp: String(isValid) }));
            return;
        }
        isValid = validator.password(password);
        if (isValid != true) {
            setHelpText((prev) => ({ ...prev, signUp: String(isValid) }));
            return;
        }
        isValid = validator.confirmPassword(password, confirmPassword);
        if (isValid != true) {
            setHelpText((prev) => ({ ...prev, signUp: String(isValid) }));
            return;
        }
        isValid = validator.input(name, "Name");
        if (isValid != true) {
            setHelpText((prev) => ({ ...prev, signUp: String(isValid) }));
            return;
        }
        isValid = validator.phone(phone);
        if (isValid != true) {
            setHelpText((prev) => ({ ...prev, signUp: String(isValid) }));
            return;
        }

        try {
            setLoaing((prev) => ({ ...prev, signUp: true }));
            const code = await handleVerifyEmail(email) || "";
            setVerifyCode(code.toString());
            setOpenVerifyDialog(true);
        } finally {
            setLoaing((prev) => ({ ...prev, signUp: false }));
        }
    };

    const handleResendVerifyCode = async (email: string) => {
        try {
            setLoaing((prev) => ({ ...prev, resendCode: true }));
            const code = await handleVerifyEmail(email) || "";
            setVerifyCode(code.toString());
        } finally {
            setLoaing((prev) => ({ ...prev, resendCode: false }));
        }
    };

    const handleInputVerifyCode = async (value: string) => {
        setHelpText((prev) => ({ ...prev, resendCode: "" }));
        if (value.length !== 6) return;

        if (value != verifyCode) {
            setHelpText((prev) => ({ ...prev, resendCode: "Wrong code!" }));
            return;
        }

        try {
            await fetchSignUp();
        } finally {
            setHelpText((prev) => ({ ...prev, resendCode: "" }));
            setVerifyCode("");
            setOpenVerifyDialog(false);
        }
    };

    const handleVerifyEmail = async (email: string) => {
        try {
            const code = await fetchVerifyEmail(email);
            return code;
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (error: any) {
            if (error.response?.status === 400) {
                setHelpText((prev) => ({ ...prev, signUp: "Invalid email address." }));
            } else {
                setHelpText((prev) => ({
                    ...prev,
                    signUp: "Something went wrong. Please try again."
                }));
            }
        }
    };

    const fetchSignUp = async () => {
        const newUser = JSON.parse(JSON.stringify(valueSignUp));
        delete newUser.confirmPassword;
        try {
            const res = await axiosPublic.post("/auth/sign-up", newUser);
            if (res.data.message === "success") naviagte("/sign-in");
            setHelpText((prev) => ({
                ...prev,
                signUp: "Something went wrong. Please try again."
            }));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (error: any) {
            if (error.response?.status === 409) {
                setHelpText((prev) => ({ ...prev, signUp: "This email has been registered." }));
            } else {
                setHelpText((prev) => ({
                    ...prev,
                    signUp: "Something went wrong. Please try again."
                }));
            }
        }
    };

    return (
        <>
            <div
                className="min-h-[900px] bg-cover bg-center relative"
                style={{ backgroundImage: `url(${SignUpBgLg})` }}>
                {/* Overlay */}
                <div className="w-full h-full absolute inset-0 bg-black opacity-50"></div>

                {/* Form sign up - centered */}
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md px-6">
                    <div className="bg-white p-8 rounded-lg shadow-2xl">
                        <h1 className="text-2xl font-semibold mb-6 text-center">Sign Up</h1>

                        {/* Local sign up */}
                        <form className="space-y-4" onSubmit={handleSignUp}>
                            <Input
                                type="email"
                                placeholder="Email"
                                value={valueSignUp.email}
                                onChange={(e) => handleChangeInput(e.target.value, "email")}
                            />
                            <Input
                                type="password"
                                placeholder="Password"
                                value={valueSignUp.password}
                                onChange={(e) => handleChangeInput(e.target.value, "password")}
                            />
                            <Input
                                type="password"
                                placeholder="Confirm Password"
                                value={valueSignUp.confirmPassword}
                                onChange={(e) =>
                                    handleChangeInput(e.target.value, "confirmPassword")
                                }
                            />
                            <Input
                                type="text"
                                placeholder="Name"
                                value={valueSignUp.name}
                                onChange={(e) => handleChangeInput(e.target.value, "name")}
                            />
                            <Input
                                type="text"
                                placeholder="Phone"
                                value={valueSignUp.phone}
                                onChange={(e) => handleChangeInput(e.target.value, "phone")}
                            />
                            <Select
                                value={valueSignUp.role}
                                onValueChange={(value) => handleChangeInput(value, "role")}>
                                <SelectTrigger className="w-full">
                                    <SelectValue placeholder="Who do you want to be?" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                                        <SelectLabel>User or Owner</SelectLabel>
                                        <SelectItem value="user">User</SelectItem>
                                        <SelectItem value="owner">Owner</SelectItem>
                                    </SelectGroup>
                                </SelectContent>
                            </Select>
                            <span className="text-sm text-red-500 text-left">
                                {helpText.signUp}
                            </span>
                            {loading.signUp ? (
                                <Button className="mt-2 w-full cursor-pointer h-10" size="icon">
                                    <LoaderCircle className="animate-spin" />
                                </Button>
                            ) : (
                                <Button className="mt-2 w-full cursor-pointer h-10" type="submit">
                                    Sign Up
                                </Button>
                            )}
                        </form>
                        <div className="mt-3 flex justify-end items-start gap-2 text-gray-500 text-sm">
                            <Link to="/sign-in" className="hover:text-black">
                                Have an account before?
                            </Link>
                        </div>

                        <div className="mb-5 mt-2 flex justify-center items-center w-full gap-2">
                            <hr className="w-full border-[1px] border-gray-200" />
                            <span className="mb-1 text-gray-500">or</span>
                            <hr className="w-full border-[1px] border-gray-200" />
                        </div>

                        {/* Google login */}
                        <div className="flex flex-col justify-start items-center gap-2 w-full">
                            <Button
                                variant="ghost"
                                className="w-full flex justify-center items-center border-[1px] h-10 cursor-pointer">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    x="0px"
                                    y="0px"
                                    width="100"
                                    height="100"
                                    viewBox="0 0 48 48">
                                    <path
                                        fill="#FFC107"
                                        d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path>
                                    <path
                                        fill="#FF3D00"
                                        d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
                                    <path
                                        fill="#4CAF50"
                                        d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"></path>
                                    <path
                                        fill="#1976D2"
                                        d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
                                </svg>
                                Login with Google
                            </Button>
                        </div>
                    </div>
                </div>
            </div>

            <Dialog open={openVerifyDialog} onOpenChange={setOpenVerifyDialog}>
                <form>
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>Verify Email</DialogTitle>
                            <DialogDescription>We have sent a code to your emai.</DialogDescription>
                        </DialogHeader>
                        <div className="w-full flex flex-col justify-center items-center">
                            <InputOTP maxLength={6} onChange={handleInputVerifyCode}>
                                <InputOTPGroup>
                                    {[...Array(6)].map((_, i) => (
                                        <InputOTPSlot key={i} index={i} />
                                    ))}
                                </InputOTPGroup>
                            </InputOTP>
                            <span className="mt-2 text-sm text-red-700">{helpText.resendCode}</span>
                        </div>
                        <DialogFooter>
                            <DialogClose asChild>
                                <Button
                                    variant="outline"
                                    onClick={() => setOpenVerifyDialog(false)}>
                                    Cancel
                                </Button>
                            </DialogClose>
                            {loading.resendCode ? (
                                <Button size="icon" disabled>
                                    <LoaderCircle className="animate-spin" />
                                </Button>
                            ) : (
                                <Button onClick={() => handleResendVerifyCode(valueSignUp.email)}>
                                    Resend Code
                                </Button>
                            )}
                        </DialogFooter>
                    </DialogContent>
                </form>
            </Dialog>
        </>
    );
}

export default SignUp;
